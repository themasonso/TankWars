...
namespace NetworkUtil
{
    ...
    public static class Networking
    {
        ...
        public static TcpListener StartServer(Action<SocketState> toCall, int port)
        {
            ...
            Tuple<TcpListener, Action<SocketState>> data = new Tuple<TcpListener, Action<SocketState>>(listner, toCall);
            listner.Start();

            listner.BeginAcceptSocket(AcceptNewClient, data);
        ...
        {
            Tuple<TcpListener, Action<SocketState>> data = (Tuple<TcpListener, Action<SocketState>>)ar.AsyncState;
            TcpListener listener = (TcpListener)data.Item1;
            Action<SocketState> toCall = (Action<SocketState>)data.Item2;
            ...
            try
            {
                ...
                listener.BeginAcceptSocket(AcceptNewClient, data);
            } catch (Exception e)
            {
                toCall(new SocketState(toCall, null) { ErrorOccurred = true, ErrorMessage = e.Message });
            }
        }
        ...
        public static void ConnectToServer(Action<SocketState> toCall, string hostName, int port)
        {
            ...
            try
            {
                ...
                if (!foundIPV4)
                {
                    toCall(new SocketState(toCall, null) { ErrorOccurred = true, ErrorMessage = "Could not find an IPV4 address" });
                }
            }
            catch (Exception)
            {
                ...
                {
                    toCall(new SocketState(toCall, null) { ErrorOccurred = true, ErrorMessage = "Host name is not a valid IP Address" });
                }
            }
        ...
        private static void ConnectedCallback(IAsyncResult ar)
        {
            ...
            {
                state.OnNetworkAction(new SocketState(state.OnNetworkAction, null) { ErrorOccurred = true, ErrorMessage = "Something went wrong while ending the connection" });
            }
        }
        ...
        private static void ReceiveCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                if(numBytes == 0)
                {
                    ...
                    state.ErrorMessage = "0 bytes received";
                    state.OnNetworkAction(state);
                }
                string data = Encoding.UTF8.GetString(state.buffer, 0, numBytes);
                string buffer = state.GetData();
                lock (buffer)
                {
